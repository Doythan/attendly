...
export async function POST(req: NextRequest) {
  ...
  const results: { id: string; status: 'SENT' | 'FAILED'; messageId?: string; error?: string }[] = []
  ...
  for (const messageId of messageIds) {
    ...
    if (!student) {
      await supabase.from('messages').update({ status: 'FAILED', error: 'Student not found' }).eq('id', messageId)
    ...
    if (!solapiRes.ok || !solapiData.messageId) {
      ...
      await supabase.from('messages').update({ status: 'FAILED', error: errMsg }).eq('id', messageId)
      results.push({ id: messageId, status: 'FAILED', error: errMsg })
    } else {
      await supabase
        .from('messages')
        .update({ status: 'SENT', provider_message_id: solapiData.messageId, error: null })
        .eq('id', messageId)
      results.push({ id: messageId, status: 'SENT', messageId: solapiData.messageId })
    }
  }
